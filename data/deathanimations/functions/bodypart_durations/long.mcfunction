...
execute if score low_p_entities entity_count matches 50.. run scoreboard players add @e[type=#deathanimations:marker_entities,tag=low_p_mode] timeout_parts 1
#execute if score low_p_entities entity_count matches 250.. run function deathanimations:clear

#execute if score global entity_count matches 40.. run function deathanimations:clear

execute as @e[type=#deathanimations:marker_entities,tag=visual,scores={age=35..},tag=OnGround,tag=!playerbody] at @s unless entity @s[scores={age=145..150}] unless entity @a[distance=..3] run function deathanimations:low_p_mode/activate
