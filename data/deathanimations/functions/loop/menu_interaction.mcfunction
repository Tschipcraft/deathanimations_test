...
execute as @a if entity @s[scores={blood_local=3}] run tellraw @s ["",{"text":"=-=Death Animations v1.2 by Tschipcraft installed!=-=","bold":true,"color":"aqua"},{"text":"\n"},{"text":"For settings issue the command","color":"green"},{"text":"\n"},{"text":"/trigger settings","italic":true,"color":"green","clickEvent":{"action":"suggest_command","value":"/trigger settings"},"hoverEvent":{"action":"show_text","value":"Or click here"}}]
execute as @a if entity @s[scores={blood_local=3}] run tellraw @s ["",{"text":"[Death Animations]","color":"gray"},{"text":" Make sure to update or install the resource pack for this release!","color":"aqua"}]
