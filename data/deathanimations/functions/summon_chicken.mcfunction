...
summon minecraft:armor_stand ^ ^ ^ {Small:1b,Tags:[bodypart,chickenbody,lame_bodypart],Silent:1,Invisible:1,DisabledSlots:4144959}
execute at @s run tp @s ~ ~-1.4 ~
data merge entity @s {Pose:{Head:[0f,0f,0f]},Small:0b,Marker:0,NoGravity:1,ArmorItems:[{},{},{},{id:"minecraft:carrot_on_a_stick",Count:1,tag:{CustomModelData:9,display:{Name:"{\"text\":\"Chicken\",\"italic\":false}"}}}]}
scoreboard players add @s cooldown 1
execute at @s positioned ~ ~1.4 ~ run scoreboard players add @e[tag=chickenbody,limit=1,sort=nearest] cooldown 1
...
execute if score Global enable_drops matches 1 run loot spawn ~ ~ ~ loot minecraft:entities/chicken
