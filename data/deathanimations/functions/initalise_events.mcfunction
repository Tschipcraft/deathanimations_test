...
execute as @s[type=player] as @e[type=#deathanimations:projectiles,type=!trident,nbt={inGround:0b},distance=..25] at @s run summon minecraft:area_effect_cloud ~ ~ ~ {Tags:[arrow,d_a_p],Radius:0f,RadiusPerTick:0f,Duration:6}
...
execute as @s[type=player] as @e[type=tnt,distance=..25] at @s unless entity @e[type=#deathanimations:marker_entities,tag=d_a_p,distance=..0.1] run summon minecraft:area_effect_cloud ~ ~ ~ {Tags:[explosion,d_a_p],Radius:0f,RadiusPerTick:0f,Duration:5}
